...
int natality(LandScape *L, LinkList *Population, Queue *Dispersers, Parameters *Pm){
    ...
    for(CurInd = Population->NextInd; CurInd->Age >= 0; CurInd = CurInd->NextInd){
        ...
        if (rand0to1() <= birthrate){
            ...
            Baby->avghabquality = habitatvalue(GetValue(L,Pm->SurvLayer, Baby->Where),HabValue,Pm->MaxAgeClass);
...
int dispersal(LandScape *L, LinkList *Population, Queue *Dispersers, Parameters *Pm){
    ...
    Individual *individualLocked = NULL;

#pragma omp parallel private(Returned, Displaced, NewCell, individualLocked) reduction(+:dispersecount, repeatcount, killcount)
    {
        while((Returned = DeQueue(Dispersers)) != NULL){
            ...
            while(Returned){
                ...
                if (rand0to1() <= Pm->DispMort){
                    ...
                    // Just one thread for each cell and for each indivual
                    omp_set_lock(&L->CellLocks[NewCell]);

                    individualLocked = (Individual*)(L->Map[NewCell]);
                    if (individualLocked)
                    {
                        omp_set_lock(&individualLocked->mp_indiv_lock);
                    }

                    switch ((accept[Returned->selectortype])(L,NewCell,Returned,Pm))
                    {   /*accept is a POINTER to a function */
                    case -1 : /* displace previous owner */
                        Displaced = (Individual*)L->Map[NewCell];
                        Displaced->numbumped++;
                        Returned->numbumps++;
                    ...
                    if(individualLocked)
                    {
                        omp_unset_lock(&individualLocked->mp_indiv_lock);
                    }

                    omp_unset_lock(&L->CellLocks[NewCell]);
                }   /* else survived */
            }   /* end while loop */
...
int accept0(LandScape *L, Location NewCell, Individual *newbie, Parameters *Pm)
{
    ...
    LandCell cell = L->Map[NewCell];

    if (cell)    /* if cell occupied... */
    {
        if (((Individual*)(cell))->Age > newbie->Age)   /* ...by someone older */
    ...
    if (cell)    /* cell occupied by another newborn */
    {
...
int accept1(LandScape *L, Location NewCell, Individual *newbie, Parameters *Pm)
{
    ...
    */
    ...
    if (cell)
    {
        if (((Individual*)(cell))->Age > newbie->Age)
            /*        printf("%i\n",NewCell);*/
            return 0;   /*reject occupied cell*/
    }
